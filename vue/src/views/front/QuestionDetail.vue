            ...
            <div>
                <div v-for="item in answerList" :key="item.id" style="display: flex; grid-gap: 10px; padding-bottom: 20px; border-bottom: 1px solid #ddd; margin-bottom: 20px">
                    <div style="color: #666; padding-top: 20px; width: 60px; cursor: pointer">
                        <span @click="addLikes(item.id)" :style="{'color':item.likes?'red':'#666'}">
                        <i style="font-size: 20px" class="el-icon-like" ></i>
                        {{item.likesCount}}
                        </span>
                    </div>
                    <div style="flex: 1;">
...
export default {
    ...
    methods: {
        ...
        addLikes(answerId){
          this.$request.post('/likes/add',{fid: answerId,module:'answer'}).then(
              res=>{
                  if(res.code === '200'){
                      this.loadAnswer(1)
                  }else{
